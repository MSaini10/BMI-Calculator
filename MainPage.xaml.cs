...
namespace BMI
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async void btnCalc_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (await isValid())
                {
                    ...
                    if (!metricUnits)
                    {
                        bmi = BMIStatic.bmiValue((h * 12 + inch), w, SysOfUnits.Imperial, out category);
                    }
                    else
                    {
                        bmi = BMIStatic.bmiValue(h, w, SysOfUnits.Metric, out category);
                    }
            ...
            {
                App.ShowMessage("Calculation Error", ax.ParamName);
            }
            catch (Exception ex)
            {
        ...
        private async Task<bool> isValid()
        {
            ...
            {
                App.ShowMessage("Invalid Data", errMessage);
                if (playSpeech)
                {
